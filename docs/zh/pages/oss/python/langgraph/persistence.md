...
大多数应用程序都可以使用两者：[checkpointer](/oss/python/langgraph/checkpointers) 跟踪当前线程，[store](/oss/python/langgraph/stores) 跟踪跨线程的持久信息。
...
* [Use checkpointers](/oss/python/langgraph/checkpointers) 保存并检查线程状态。
* [Use stores](/oss/python/langgraph/stores) 跨线程持久保存数据。
