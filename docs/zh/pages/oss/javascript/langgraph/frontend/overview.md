...
[LangChain frontend patterns](/oss/javascript/langchain/frontend/overview)——markdown 消息、工具调用、人机交互、可恢复流和时间旅行——适用于任何 LangGraph 图。无论您使用 `createAgent`、`createDeepAgent` 还是自定义 `StateGraph`，流 API 都提供相同的核心数据模型。
